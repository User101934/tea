...
const Hero = () => {
  ...
  useEffect(() => {
    // Increased interval to account for slower typing/erasing speeds
              ...
              <span className="relative inline-flex items-center">
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    transition={{
                      width: { duration: 1.2, ease: [0.4, 0, 0.2, 1] }, // Slower typing speed
                      opacity: { duration: 0.4 }
                    }}
                    className="inline-flex items-center bg-gradient-to-b from-[#1a1a1a] to-[#000000] text-white px-3 py-0.5 sm:px-4 sm:py-1 rounded-md sm:rounded-lg shadow-xl overflow-hidden whitespace-nowrap border border-white/20"
                    style={{ fontSize: '0.82em' }}
                  >
                    <span className="flex items-center font-medium tracking-tight">
                      {current.text}
                      <span
                        className="inline-block w-[2px] h-[0.85em] bg-white ml-1.5"
